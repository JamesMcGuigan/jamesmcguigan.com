...
d3.queue()
  ...
  .await(function(error, primary_debates_parsed) {
    ...
    d3.select('#topWordsByParty thead')
      .append('tr')
      .selectAll('th')
      .data(_.keys(primary_debates_parsed.groupBy.Party))
      .enter()
        .append('th')
        .text((d) => d)
    ;
    ...
    var wordArray = _(0).range(100).map(function(i) {
      return _(primary_debates_parsed.groupBy.Party)
        .values()
        .map((words) => _(words).keys().get(i) )
        .value();
    }).value();

    d3.select('#topWordsByParty tbody')
      .selectAll('tr')
      .data(wordArray)
        .enter()
        .append('tr')
